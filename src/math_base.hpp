...
static constexpr std::enable_if_t<std::is_floating_point_v<value_type>,
                                  value_type>
    pi = static_cast<value_type>(M_PI);

template<typename value_type>
inline constexpr std::enable_if_t<std::is_arithmetic_v<value_type>, value_type>
sign(value_type value)
{
    return value_type(value >= 0 ? 1 : -1);
}
template<typename value_type>
inline std::enable_if_t<std::is_floating_point_v<value_type>, value_type>
simplify_angle(value_type theta)
{
    return std::atan2(std::sin(theta), std::cos(theta));
}
template<typename value_type>
inline constexpr std::enable_if_t<std::is_floating_point<value_type>::value,
                                  value_type>
unit_circle(value_type theta)
{
    return value_type(simple_angle(theta) > 0
                          ? simple_angle(theta)
                          : simple_angle(theta) + 2 * pi<value_type>);
}
...
{
    value_type result = 0;
    for (uint64_t i = start; i < end; ++i) { result += function(i); }
    return result;
}
template<typename value_type>
inline constexpr std::enable_if_t<std::is_floating_point_v<value_type>,
                                  value_type>
...
template<typename value_type>
inline constexpr std::enable_if_t<std::is_floating_point_v<value_type>, bool>
