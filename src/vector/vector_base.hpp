...
class __vector_private::__vector_base
{
    ...
    inline __attribute__((always_inline)) auto& array()
    {
        return __data;
    }
    inline __attribute__((always_inline)) const auto& array() const
    {
        return __data;
    }
    ...
    inline __attribute__((always_inline)) value_type
    dot(const_self_reference other)
    {
        value_type result = 0;
        for (size_t i = 0; i < dim; ++i) { result += (at(i) * other.at(i)); }
        return result;
    }
...
operator!(const vector<value_type, dim>& vector)
{
    return !(bool(vector));
}
template<typename value_type, size_t dim>
inline __attribute__((always_inline)) vector<value_type, dim>
operator+(const vector<value_type, dim>& one,
          const vector<value_type, dim>& two)
{
           ...
           const vector<value_type, dim>& two)
{
    ...
    {
        if (one.at(i) != two.at(i)) { return false; }
    }
    return true;
}
template<typename value_type, size_t dim>
...
std::ostream& operator<<(std::ostream&                  ostream,
                         const vector<value_type, dim>& vec)
{
    std::string str;
    str += "<";
    for (const auto& i : vec.array()) { str += std::to_string(i) + ", "; }
    str = str.substr(0, str.length() - 2);
    str += ">";
    return ostream << str;
}
template<typename value_type, size_t dim>
value_type dot(const vector<value_type, dim>& one,
               const vector<value_type, dim>& two)
{
    return one.dot(two);
}
template<typename value_type, size_t dim>
value_type distance(const vector<value_type, dim>& one,
                    const vector<value_type, dim>& two)
{
...
template<typename value_type, size_t dim>
value_type angle(const vector<value_type, dim>& one,
                 const vector<value_type, dim>& two)
{
    if (!one || !two)
        throw divide_by_zero("Cannot find angle for zero vector(s)");
    return acos(dot(one, two) / (one.norm() * two.norm()));
}

template<typename value_type, size_t dim>
inline __attribute__((always_inline)) vector<value_type, dim>
projection(const vector<value_type, dim>& vec,
           const vector<value_type, dim>& normal)
{
    if (!vec) throw divide_by_zero("Cannot project zero vector");
    if (!normal) throw divide_by_zero("Cannot project over zero normal");
    return (dot(vec, normal.normalized()) * normal.normalized());
}

template<typename value_type, size_t dim>
vector<value_type, dim> reflection(const vector<value_type, dim>& vec,
                                   const vector<value_type, dim>& normal)
{
    if (!vec) throw divide_by_zero("Cannot reflect zero vector");
    if (!normal) throw divide_by_zero("Cannot reflect over zero normal");
