                               ...
                               value_type                   angle)
{
    ...
    return (vec * cos(angle) + cross(vec, axis.normalized()) * sin(angle) +
            projection(vec, axis) * (1 - cos(angle)));
}

template<typename value_type>
inline __attribute__((always_inline)) bool
operator==(const vector<value_type, 3>& one, const vector<value_type, 3>& two)
{
