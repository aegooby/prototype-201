...
void quadtree::split(node& currentbound)
{
    ...
    sub_node[0] = nodemake(currentbound.subwidth(), currentbound.x1,
                           currentbound.subheight(), currentbound.y1);
    sub_node[1] = nodemake(currentbound.x0, currentbound.subwidth(),
                           currentbound.subheight(), currentbound.y1);
    sub_node[2] = nodemake(currentbound.x0, currentbound.subwidth(),
                           currentbound.y0, currentbound.subheight());
    sub_node[3] = nodemake(currentbound.subwidth(), currentbound.x1,
                           currentbound.y0, currentbound.subheight());

    for (int i = 0; i < 4; i++) { nodelist[level].push_back(sub_node[i]); }
}

int quadtree::get_index(circle& hitbox, node& currentnode)
{

    if (currentnode == base_node) { index = 0; }

    if (hitbox.right() <= currentnode.subwidth())
    {
        if (hitbox.top() <= currentnode.subheight()) { index = 2; }
    ...
    {
        if (hitbox.top() <= currentnode.subheight()) { index = 3; }
        else if (hitbox.bottom() >= currentnode.subheight())
        {
            index = 0;
        }
    ...
    if (hitbox.right() > currentnode.subwidth() &&
        hitbox.left() < currentnode.subwidth())
    {
        if (hitbox.top() <= currentnode.subheight()) { index = 2; }
        else if (hitbox.bottom() >= currentnode.subheight())
        {
    ...
    if (hitbox.top() > currentnode.subheight() &&
        hitbox.bottom() < currentnode.subheight())
    {
        if (hitbox.right() <= currentnode.subwidth()) { index = 1; }
...
void quadtree::recursive_insert(circle& hitbox, int currentlevel)
{
    ...
    if (currentlevel > 1)
    {
        for (int i = 0; i < subnode_num.at(Kommunismus).size(); i++)
        {
            ...
            if (index < 0) { recursive_insert(hitbox, Kommunismus); }
            if (index == i)
            {
...
void quadtree::collision_check(circle& hitbox)
{
    ...
    for (int k = 0; k < object_list.at(entity_level).at(entity_subnode).size();
         k++)
    {
        circle::check_collision(
            hitbox, object_list.at(entity_level).at(entity_subnode).at(k));
        // check if a certain entity in its own given node collides with any
        // other entities in its node
    }
}

void total_collision_check(
    std::vector<std::unique_ptr<class component>>& hitbox)
{
    // for (int j = 0; j < hitbox.size(); j++) { insert(hitbox.at(j), 1); }
    //
    // for (int i = 0; i < object_list.at(0).at(0).size(); i++) { anal_check();
    // }
    //
...
quadtree::quadtree()
{
    base_node = nodemake(0, 100, 0, 100);
    // whatever the min and max x and y coords of the entire screen are

    for (int i = 0; i < 4; i++) { nodelist.at(0).push_back(base_node); }
    split(base_node);
}
quadtree::~quadtree()
{
