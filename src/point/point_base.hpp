...
public:
    ...
    __point_base(const std::initializer_list<value_type>& list)
    {
        if (list.size() > dim)
            throw std::overflow_error(
                "p201::point: Initializer list is larger than point size");
        __data = list;
    }
    ...
    {
        if (index >= dim)
            throw std::out_of_range("p201::point::at: Element is out of range");
...
operator!(const point<value_type, dim>& point)
{
    return !(bool(point));
}
template<typename value_type, size_t dim>
...
std::ostream& operator<<(std::ostream&                 ostream,
                         const point<value_type, dim>& pt)
{
    std::string str;
    str += "(";
    for (const auto& i : pt.array()) { str += std::to_string(i) + ", "; }
