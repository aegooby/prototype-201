...
public:
    ...
    using self_type              = array<value_type, __size>;
...
private:
    inline __attribute__((always_inline)) void allocate(size_type size)
    {
        if (size > std::numeric_limits<difference_type>::max())
            throw std::length_error("p201::Array: size is too large");
        __data = memory::allocate(__data, size);
    }
    template<class... types>
    inline __attribute__((always_inline)) void construct(size_type offset,
                                                         types&&... args)
    {
        memory::construct(iterator(__data) + offset,
                          std::forward<types>(args)...);
    }
    inline __attribute__((always_inline)) void destruct(size_type offset)
    {
        memory::destruct(iterator(__data) + offset);
    }
    inline __attribute__((always_inline)) void deallocate()
    {
...
public:
    ...
    constexpr array(types&&... args)
    {
        ...
        if constexpr (!sizeof...(args)) { this->fill(value_type()); }
    }
    ~array()
    {
        for (size_t i = __size - 1; i + 1 >= 1; --i) this->destruct(i);
        this->deallocate();
    }
    ...
    array(const st_array<value_type, __size>&);
    array(st_array<value_type, __size>&&);
    inline __attribute__((always_inline)) self_reference
    operator=(const_self_reference other) noexcept
    {
        memcpy(__data, other.__data,
               (__size > 0 ? __size : 1) * sizeof(value_type));
        return *this;
    ...
    inline __attribute__((always_inline)) self_reference
    operator=(self_rvalue_reference other) noexcept
    {
        memcpy(__data, other.__data,
               (__size > 0 ? __size : 1) * sizeof(value_type));
        return *this;
    }
    self_reference operator=(const st_array<value_type, __size>&);
    self_reference operator=(st_array<value_type, __size>&&);
    array(const std::initializer_list<value_type>& list)
    {
        if (list.size() != __size &&
            !std::is_default_constructible_v<value_type>)
            throw std::runtime_error("p201::Array: Initializer list leaves "
                                     "elements unininitialized");
        if (list.size() > __size)
    ...
    inline __attribute__((always_inline)) const pointer data() const noexcept
    {
        return __data;
    }
